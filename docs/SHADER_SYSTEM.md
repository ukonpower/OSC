...
| **texture[2]** | RGBA8 | outColor.r | outColor.g | outColor.b | **0.0 (未使用)** | アルベドカラー + **未使用チャンネル** |
...
1. **texture[2].a** (アルベドのアルファチャンネル) - 常に`0.0`が書き込まれている → **完全に未使用**
   - 形式: 8bit (0-255の整数値)
   - 用途例: 透明度、マスク、フラグ情報

2. **texture[4].b** (モーションベクターのZ成分) - 常に`0.0`が書き込まれている → **完全に未使用**
   - 形式: 32bit浮動小数点
   - 用途例: 深度情報、高精度パラメータ
